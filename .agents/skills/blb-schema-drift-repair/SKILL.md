...
1. Start with a current drift report. Prefer `php artisan blb:schema:drift` when that command is available. If it is unavailable, or the report has unreadable/unchecked operations, stop and report that no automatic repair decision is safe.
2. Confirm both `APP_ENV` and the report's database identity. Mutate only `local` or `testing` databases. For production, staging, external, or ambiguous databases, make no database change; explain the required forward migration or operator action.
...
4. Re-run the drift report after the migration succeeds. Verify every original finding is resolved and that the report contains no newly unchecked source. Run focused migration or feature tests when source changed.
