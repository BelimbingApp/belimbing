...
# Install FrankenPHP/Caddy's internal CA into the system trust store (tls internal fallback only).
# This is only needed when BLB falls back to `tls internal` (mkcert certs missing).
# The normal dev path uses mkcert certs whose CA is trusted by `mkcert -install` at start time.
# Call this manually via scripts/setup-steps/75-ssl-trust.sh if you encounter browser warnings.
...
setup_ssl_trust() {
    ...
    if [[ "$installed" = false ]]; then
        ...
        echo -e "  Or switch to mkcert certs: ${CYAN}./scripts/setup-steps/70-domains.sh${NC}"
    fi
...
# mkcert is the primary dev path — `mkcert -install` (called at start time) handles all
