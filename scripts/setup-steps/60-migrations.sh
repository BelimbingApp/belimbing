...
#
# This script:
# - Prompts for framework primitives (platform-operator company, admin user)
...
#
# Framework primitives (platform operator, primary company, admin user, Lara) are created by
# FrameworkPrimitivesProvisioner (called from MigrateCommand) in all environments.
# Tenant and company IDs are generated by the database and carry no role meaning.
#
# Prerequisites:
...
load_existing_framework_primitives() {
    ...
    BLB_PREFERRED_ADMIN_USER_ID="$preferred_user_id" php artisan tinker --execute='
        $explicitTenancy = Illuminate\Support\Facades\Schema::hasColumn("tenants", "is_platform_operator")
            && Illuminate\Support\Facades\Schema::hasTable("tenant_primary_companies");
        $company = $explicitTenancy
            ? app(App\Core\Company\Services\PrimaryCompanyManager::class)->findForTenant(App\Base\Tenancy\Models\Tenant::requirePlatformOperator())
            : App\Core\Company\Models\Company::query()->find(1); // Legacy pre-migration input only.
...
main() {
    ...
    # Check if framework primitives (operator primary company + admin user) already exist in the DB.
    ...
    if [[ "$primitives_exist" = false ]]; then
        # Prompt for framework primitives (platform-operator company, admin user).
        ...
        # LICENSEE_* setup-state keys are a bounded upgrade fallback only.
        company_name_default=$(get_setup_state_var "PLATFORM_OPERATOR_COMPANY_NAME" "$(get_setup_state_var "LICENSEE_COMPANY_NAME" "${company_name:-My Company}")")
        if [[ -t 0 ]]; then
            company_name=$(ask_input "Platform-operator company name" "$company_name_default")
        else
        ...
        company_code_default=$(get_setup_state_var "PLATFORM_OPERATOR_COMPANY_CODE" "$(get_setup_state_var "LICENSEE_COMPANY_CODE" "${company_code:-$default_code}")")
        if [[ -t 0 ]]; then
            company_code=$(ask_input "Platform-operator company code" "$company_code_default")
        else
    ...
    if ! PLATFORM_OPERATOR_COMPANY_NAME="$company_name" \
            PLATFORM_OPERATOR_COMPANY_CODE="$company_code" \
    ...
    persisted_admin_user_id=$(php artisan tinker --execute='
        $company = app(App\Core\Company\Services\PrimaryCompanyManager::class)->platformOperatorCompany();
    ...
    save_to_setup_state "PLATFORM_OPERATOR_COMPANY_NAME" "$company_name"
    save_to_setup_state "PLATFORM_OPERATOR_COMPANY_CODE" "$company_code"
    remove_from_setup_state "LICENSEE_COMPANY_NAME"
    remove_from_setup_state "LICENSEE_COMPANY_CODE"
