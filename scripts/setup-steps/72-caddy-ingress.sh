...
# Copy the published static error page next to the certs so the caddy service
# user can serve it while the BLB instance is down or mid-update — the project
# tree may be unreadable to system daemons, or in the middle of being swapped.
# Prints the destination directory on stdout.
provision_error_pages_for_system_caddy() {
    local caddyfile_path=$1
    local src_page="$PROJECT_ROOT/public/errors/5xx.html"
    local include_dir
    include_dir=$(get_system_caddy_include_dir "$caddyfile_path")
    local dest_dir="${include_dir}/errors"
    local dest_page="${dest_dir}/5xx.html"

    copy_file_with_elevation_if_needed "$src_page" "$dest_page"
    chmod 644 "$dest_page" 2>/dev/null || sudo chmod 644 "$dest_page"
...
generate_site_fragment() {
    ...
    local errors_dir="${4:-$PROJECT_ROOT/public/errors}"
    local fragment_path
    ...
    {
        printf '# Generated by scripts/setup-steps/72-caddy-ingress.sh for %s\n' "$PROJECT_ROOT"
        printf '# BLB system Caddy integration for %s.\n\n' "$APP_ENV"
        caddy_render_system_site_block "$frontend_domain" "$tls_directive" "$app_port" "$errors_dir"
    } > "$fragment_path"
...
configure_shared_ingress() {
    ...
    # The branded "server unavailable" page must stay readable by the caddy
    # service user even while this project tree is being swapped or updated.
    local system_errors_dir="$PROJECT_ROOT/public/errors"
    if [[ -f "$PROJECT_ROOT/public/errors/5xx.html" ]]; then
        system_errors_dir=$(provision_error_pages_for_system_caddy "$caddyfile_path")
        echo -e "${GREEN}✓${NC} Provisioned fallback error page for system Caddy: ${CYAN}${system_errors_dir}/5xx.html${NC}"
    fi

    local fragment_path
    fragment_path=$(generate_site_fragment "$frontend_domain" "$app_port" "$system_cert_dir" "$system_errors_dir")
