...
$configuredCaddyBindAddress = Get-EnvValue $envPath 'CADDY_BIND_ADDRESS' ''
$usesAutomaticDataShareLanListener = [string]::IsNullOrWhiteSpace($configuredCaddyBindAddress)
$caddyBindAddress = if ($usesAutomaticDataShareLanListener) { '0.0.0.0' } else { $configuredCaddyBindAddress }
$dataShareLanAvailable = $caddyBindAddress -notin @('127.0.0.1', '::1', 'localhost')
...
$env:CADDY_REMOTE_ACCESS_SNIPPET = if ($usesAutomaticDataShareLanListener) { 'scripts/caddy-snippets/remote-access-data-share-only.caddy' } else { 'scripts/caddy-snippets/remote-access-open.caddy' }
$env:DATA_SHARE_LAN_CONNECTION_HINTS = if ($dataShareLanAvailable) { 'true' } else { 'false' }
