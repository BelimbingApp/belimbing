...
class BridgeImportException extends RuntimeException
{
    public static function unsafeDisk(string $disk): self
    {
        return new self(__('Bridge disk :disk is public. Configure a private filesystem disk before receiving packages.', ['disk' => $disk]));
    }

    public static function invalidSourcePath(): self
    {
        return new self(__('Choose a diagnostic package from the configured bridge diagnostics path.'));
    }

    public static function sourceMissing(string $path): self
    {
        return new self(__('Diagnostic package :path was not found on the bridge disk.', ['path' => $path]));
    }
