...
class RelationalDataSerializer
{
    ...
    public function __construct(
        private readonly BridgeSchemaFingerprint $schemaFingerprint,
        private readonly BridgeValueNormalizer $values,
        private readonly BridgeSettings $settings,
    ) {}

    public function serialize(BridgeScopeDefinition $scope): SerializedBridgeScope
    {
        $this->recordCount = 0;
        $payloads = [];
        ...
        try {
            foreach ($scope->tables as $table) {
                if ($table->primaryKeyColumns === []) {
                    throw BridgeDefinitionException::invalid(__('table :table has no primary key and cannot be bridged safely.', [
                        'table' => $table->table,
                    ]));
                }
        ...
        return new SerializedBridgeScope($payloads, [
            'tables' => count($payloads),
            'records' => $this->recordCount,
        ]);
    }

    private function serializeTable(BridgeTableDefinition $table): SerializedTablePayload
    {
    ...
    private function guardScalar(string $table, string $column, mixed $value): void
    {
        ...
        $max = $this->settings->integer('bridge.transfer_limits.max_scalar_bytes', 10 * 1024 * 1024, 1, 2147483647);

        if (strlen($value) > $max) {
            throw BridgePackageException::scalarTooLarge($table, $column, $max);
        }
    }
    ...
    private function writeLine($stream, array $value): void
    {
        ...
        $maximum = $this->settings->integer('bridge.transfer_limits.max_record_line_bytes', 32 * 1024 * 1024, 1, 2147483647);

        if (strlen($line) > $maximum) {
            throw BridgePackageException::recordLineTooLarge($maximum);
        }

        if (fwrite($stream, $line) !== strlen($line)) {
            throw BridgePackageException::payloadWriteFailed();
        }
    }
}
