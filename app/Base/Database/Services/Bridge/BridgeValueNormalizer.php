...
class BridgeValueNormalizer
{
    /** @var array<string, array<string, string>> */
    private array $types = [];

    public function __construct(private readonly BridgeSchemaFingerprint $schemas) {}
    ...
    public function encode(string $table, string $column, mixed $value): mixed
    {
        ...
        if (is_string($value) && ($this->type($table, $column) === 'binary' || ! mb_check_encoding($value, 'UTF-8'))) {
            return ['__bridge_binary_base64' => base64_encode($value)];
        }
    ...
    {
        if (is_array($value) && array_keys($value) === ['__bridge_binary_base64']) {
            $decoded = base64_decode((string) $value['__bridge_binary_base64'], true);

            if ($decoded === false) {
                throw BridgePackageException::invalidPackage(__('a binary field is not valid Base64.'));
            }
    ...
    private function json(string $table, string $column, mixed $value): string
    {
        ...
        try {
            ...
            throw BridgePackageException::invalidPackage(__('database JSON in :table.:column is invalid.', [
                'table' => $table,
                'column' => $column,
            ]));
        }
