...
class DiagnosticRowCapture
{
    ...
    private const DEFAULT_PATH_PREFIX = 'data-share/diagnostics';
    ...
    public function __construct(
        ...
        private DataShareSettings $settings,
    ) {}
    ...
    public function capture(string $table, array $ids, string $trigger, string $previewSha256): array
    {
        ...
        if (! hash_equals($previewSha256, $payloadSha256)) {
            throw DataShareCaptureException::previewChanged();
        }
        ...
        $path = $this->settings->pathPrefix('data_share.path_prefix', self::DEFAULT_PATH_PREFIX)
            .'/'.now()->format('Ymd_His').'-'.$packageId.'.json';

        $this->guardPackageSize(strlen($json));

        if (! $disk->put($path, $json)) {
            throw DataShareCaptureException::storeFailed($path);
        }
    ...
    {
        $disk = $this->disk();
        $prefix = $this->settings->pathPrefix('data_share.path_prefix', self::DEFAULT_PATH_PREFIX);
        $maxPackageBytes = $this->settings->integer('data_share.limits.max_package_bytes', 25 * 1024 * 1024, 1, 2147483647);
        $packages = [];
    ...
    {
        if ($ids === []) {
            throw DataShareCaptureException::noSelection();
        }

        $maxSelected = $this->settings->integer('data_share.limits.max_selected_rows', 100, 1, 10000);

        if (count($ids) > $maxSelected) {
            throw DataShareCaptureException::tooManySelected($maxSelected);
        }
        ...
        if ($primaryKey === null) {
            throw DataShareCaptureException::noPrimaryKey($table);
        }
    ...
    private function encodeRow(string $table, array $row): array
    {
        ...
        foreach ($row as $column => $value) {
            // Read a bytea stream so the size and encoding rules below apply
            // on every driver.
            ...
            if (strlen($value) > $maxScalarBytes) {
                throw DataShareCaptureException::scalarTooLarge($table, $column, $maxScalarBytes);
            }
    ...
    private function guardPackageSize(int $size): void
    {
        ...
        if ($size > $maxPackageBytes) {
            throw DataShareCaptureException::packageTooLarge($maxPackageBytes);
        }
    }
    ...
    private function disk(): Filesystem
    {
        ...
        if ($diskName === 'public' || ($diskConfig['visibility'] ?? null) === 'public') {
            throw DataShareCaptureException::unsafeDisk($diskName);
        }
