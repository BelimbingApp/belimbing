...
class DiagnosticPackageImporter
{
    ...
    private function validatedTables(string $contents, string $packageId): array
    {
        ...
        $maxTables = $this->settings->integer('data_share.limits.max_tables', 100, 1, 10000);
    ...
    private function assertSelection(mixed $selection, array $tables): void
    {
        if (! is_array($selection)
            ...
            || $selection['ids'] === []
            || count($selection['ids']) > $this->settings->integer('data_share.limits.max_selected_rows', 100, 1, 10000)) {
