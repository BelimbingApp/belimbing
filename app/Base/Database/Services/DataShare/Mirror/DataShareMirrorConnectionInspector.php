    ...
    public function inspect(DataShareMirrorProvider $provider, string $url, string $connectionName, bool $purgeAfter, array $availablePdoDrivers): DataShareMirrorConnectionStatus
    {
        ...
        try {
            if (! in_array((string) config('app.env'), ['local', 'testing'], true)) {
                return DataShareMirrorConnectionStatusFactory::unavailable(new DataShareMirrorConnectionContext(true, false, $driver), 'local_not_development', __('Table mirroring is available only on a development instance.'), $provider);
            }

            $local = $this->database->connection();
            $localDriver = $local->getDriverName();
            if (! in_array($localDriver, ['sqlite', 'pgsql'], true)) {
                return DataShareMirrorConnectionStatusFactory::unavailable(new DataShareMirrorConnectionContext(true, false, $driver, localDriver: $localDriver), 'local_driver_unsupported', __('The local mirror database must use SQLite or PostgreSQL.'), $provider);
            }

            $localRole = $this->identity->role()->value;
            if ($localRole !== DataShareInstanceRole::Development->value) {
                return DataShareMirrorConnectionStatusFactory::unavailable(new DataShareMirrorConnectionContext(true, false, $driver, $localRole, localDriver: $localDriver), 'local_role_denied', __('The local Data Share role must be Development.'), $provider);
            }
        ...
        try {
            ...
            if ($localDriver === 'pgsql') {
                $localInfo = $this->serverInfo($this->database->connection());
                if ($this->sameEndpoint($localInfo, $remoteInfo, $this->processConfiguration($this->database->connection()), $config)) {
                    return DataShareMirrorConnectionStatusFactory::unavailable(new DataShareMirrorConnectionContext(true, true, $driver, $localRole, serverVersion: (string) $remoteInfo['version'], localDriver: $localDriver), 'self_target', __('The mirror connection points to the local database. Choose a different development database.'), $provider);
                }
            }
            ...
            if ($remoteRole === null) {
                ...
                return DataShareMirrorConnectionStatusFactory::unavailable(
                    new DataShareMirrorConnectionContext(true, true, $driver, $localRole, $remoteRole, (string) $remoteInfo['version'], localDriver: $localDriver),
