...
class DataSharePackageRetention
{
    ...
    private function receivingCandidates(Filesystem $disk): array
    {
        $cutoff = CarbonImmutable::now('UTC')->subHours(max(
            1,
            $this->settings->integer('data_share.transfer_limits.receiving_retention_hours', 24, 1, 720),
        ));
        $prefix = $this->settings->pathPrefix('data_share.receiving_path_prefix', 'data-share/receiving');
    ...
    private function outgoingCandidates(Filesystem $disk, bool $includeUnapplied): array
    {
        ...
        return array_values(array_map(
            ...
            array_filter(
                $disk->allFiles($prefix),
                fn (string $path): bool => ! $availablePaths->has($path)
                    && $this->lastModifiedBefore($disk, $path, $cutoff),
            ),
        ));
    }
    ...
    {
        $receipt = DataShareReceipt::query()->find($receiptId);
