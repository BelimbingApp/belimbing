...
final class ScheduleStatusBarDiagnosticProvider implements StatusBarDiagnosticProvider
{
    ...
    public function diagnosticsFor(Authenticatable $user): iterable
    {
        try {
            ...
            if ($snapshot->lastRecordedActivity !== null
                && now()->subMinutes(self::RECENT_ACTIVITY_MINUTES)->gte($snapshot->lastRecordedActivity)) {
            ...
            if ($snapshot->unhealthyTasks !== []) {
                $diagnostics[] = $this->unhealthyTasksDiagnostic($snapshot->unhealthyTasks);
            }
