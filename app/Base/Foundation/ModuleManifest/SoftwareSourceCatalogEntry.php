...
/**
 * One installable software source discovered from the BelimbingApp catalog.
 *
 * Immutable value object; the catalog service builds these from cached
 * rows and returns lists to the UI.
 */
final readonly class SoftwareSourceCatalogEntry
{
    /**
    ...
    public function suggestedInstallCommand(): string
    {
        ...
        $path = $domain !== '' && $module !== ''
            ? ApplicationTopology::relativePathUnder(
                ApplicationTopology::DOMAINS,
                $this->capitalise($domain),
                $this->capitalise($module),
            )
            : ApplicationTopology::relativePathUnder(
                ApplicationTopology::DOMAINS,
                $this->capitalise($this->repoName),
            );
