...
class LaraContextProvider
{
     ...
     *
     * @return array{default_surface: string, path_convention: string, surfaces: array{platform: array{root: string}, domains: array{root_pattern: string}, extensions: array{root_pattern: string}}}
     */
    private function repositoryContext(): array
    {
        return [
            'default_surface' => 'platform',
            'path_convention' => 'Repository tool file paths are relative to target_surface. The platform surface is the project root.',
            'coding_loop' => [
            ...
            'surfaces' => [
                ...
                'domains' => [
                    'root_pattern' => 'app/Domains/<Domain>',
                ],
                'extensions' => [
                    'root_pattern' => 'app/Extensions/<Extension>',
                ],
            ],
        ];
    }
    ...
    private function installedModules(): array
    {
        ...
        foreach (ProviderRegistry::discoverModuleProviders() as $providerClass) {
            ...
            if (count($parts) >= 4 && $parts[0] === 'App' && $parts[1] === 'Core') {
                $modules[] = 'core/'.str($parts[2])->kebab()->toString();
            } elseif (count($parts) >= 5 && $parts[0] === 'App' && $parts[1] === 'Domains') {
                $domain = str($parts[2])->kebab()->toString();
                $module = str($parts[3])->kebab()->toString();
        ...
        foreach (ProviderRegistry::discoverExtensionProviders() as $providerClass) {
            ...
            if (count($parts) < 5 || $parts[0] !== 'App' || $parts[1] !== 'Extensions') {
                continue;
            }

            $owner = str($parts[2])->kebab()->toString();
            $module = str($parts[3])->kebab()->toString();
