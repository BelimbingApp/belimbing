...
class SystemInfoTool extends AbstractTool
{
    ...
    /**
     * Active non-Base module identities from the four-root topology.
     *
     * Module manifests are authoritative when present. The reader also
     * excludes disabled Domains before returning their installed modules.
     *
     * @return list<string>
     *
     * @throws ToolUnavailableException When the installed manifest inventory is invalid
     */
    private function gatherModules(): array
    {
        try {
            $moduleIds = array_keys((new ModuleManifestReader([
            ...
            return array_values(array_filter(
                $moduleIds,
                static fn (string $moduleId): bool => ! str_starts_with($moduleId, 'base/'),
            ));
        } catch (ModuleManifestException $e) {
            throw new ToolUnavailableException(
                errorCode: 'module_discovery_unavailable',
                message: 'Installed modules cannot be listed because the manifest inventory is invalid.',
                hint: $e->getMessage(),
            );
        }
    }
