...
namespace App\Core\AI\Tools\Concerns;
...
trait BuildsSurfaceToolPayload
{
    ...
    protected function repositorySurfaceSchema(string $operation): array
    {
        return [
            'type' => 'string',
            'description' => 'Repository ownership surface for file '.$operation.': "platform", "domain:<slug>", or "extension:<slug>". Defaults to "platform".',
        ];
    }
    ...
    protected function surfaceFilePayload(array $arguments): array
    {
        return [
            ...
            'target_surface' => $this->optionalString($arguments, 'target_surface') ?? 'platform',
        ];
    }
