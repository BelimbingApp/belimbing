...
Record the failed run, authored PR head, actual checkout SHAs and `domain-materialization` artifact. Compare the last green set with the failing set. A dependency is pinned in [domain-repos.json](../../scripts/ci/domain-repos.json); the Domain under test comes from the caller checkout. The caller's reusable workflow ref and `platform-ref` determine which platform code and descriptor it used. Changing platform main does not change an older immutable caller pin. Follow the [pin-advance procedure](domain-pins.md), including its paired [platform #591](https://github.com/BelimbingApp/belimbing/pull/591) / [Connector #172](https://github.com/BelimbingApp/blb-people-connector/pull/172) example.
...
The dedicated composed smoke workflow in [#604](https://github.com/BelimbingApp/belimbing/pull/604) was still open when this inventory was verified; do not assume an old caller already runs it. Existing route/table scans also have limits: cross-file route checks do not validate duplicates within one file, and table discovery reads supported literal declarations rather than arbitrary runtime DDL. See the [module authoring contract](../architecture/module-authoring.md) and [composition architecture](../architecture/module-system.md#composed-application-refusals).
