...
```php
// app/Domains/Operation/Leave/Config/authz.php
return [
    'capabilities' => [
...
Morphs store the full class name (`App\Domains\Operation\Leave\Models\LeaveApplication`), which couples the history table to PHP class paths. Using the same `flow` discriminator keeps things decoupled. Queries are simpler: `WHERE flow = 'leave_application' AND flow_id = 42`.
...
app/Domains/Operation/Leave/Workflow/
├── Guards/LeaveBalanceGuard.php        ← implements TransitionGuard
└── Actions/NotifyApplicant.php         ← implements TransitionAction

app/Domains/Operation/Logistics/Workflow/
├── Guards/HsCodeVerified.php
├── Guards/InventoryReserved.php
└── Actions/NotifyCustomsAgency.php
```

The `guard_class` and `action_class` columns in `base_workflow_status_transitions` store fully qualified class names (e.g., `App\Domains\Operation\Leave\Workflow\Guards\LeaveBalanceGuard`). The engine resolves them through Laravel's service container—no registry or custom autoloader configuration. The business Module is responsible for ensuring its guard/action classes follow the standard `App\` PSR-4 mapping.
