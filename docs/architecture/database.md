...
The Schema Incubation screen may edit and commit migration source only in the `local` environment. Production and staging render the inventory read-only: operators must make, test, and push source edits from a development checkout. This prevents an application server from accumulating server-local commits that no other fleet member can receive. The Updates screen also blocks launching any selected source whose local branch is ahead of its configured remote; push or reconcile those commits outside Belimbing and refresh source status before retrying.
...
The inspector fails closed when compared schema mutations depend on runtime values or unsupported constructs. It deliberately excludes column types/defaults, foreign keys, check constraints, triggers, and extra tables until those contracts can be proved safely. Literal raw check constraints and triggers are recognized as outside that comparison scope rather than making an otherwise replayable migration incomplete. Extra live tables are owned by the Database Residue workflow, where provenance and deletion safety can be considered separately. Drift repair follows schema maturity: local incubating source uses `migrate --dev`, while stable schema uses forward migrations.
...
-   **[docs/architecture/query-bounds.md](query-bounds.md)** — Hydration guard and the growing-table PHPStan rule: how row loading is bounded at runtime and in static analysis.
