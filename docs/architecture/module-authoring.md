...
The composed Domain CI then runs `php artisan blb:module-ownership` after every pinned Module's smoke check. This second pass compares all enabled Domain Modules, including unrelated dependency closures, and refuses duplicate table declarations, route keys, or route names while naming both owners. Extensions remain outside this pinned-Domain check.
