...
8. **Per-agent and per-task LLM model selection:** Agent workspace `config.json` keeps `llm.models[]` for chat execution and failover. Lara's workspace can additionally define `llm.tasks.*` so specialized workloads such as titling, research, coding, photo cleanup, and item-description drafting can use different models (see §15).
