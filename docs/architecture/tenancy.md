...
declared schedule list matches the live [`Illuminate\Support\Facades\Schedule`](https://laravel.com/docs/scheduling)
...
When you add a Base or Core queued job or a scheduler-registered Artisan command,
add it to that inventory first; the propagation test names any class that is
