...
- **Retrospective baseline primitive** — `captureBaseline()` can record current Local/remote counts as a labelled `mirror_baseline` run for exceptional recovery tooling. It is all-or-nothing: every selected Local/remote count is obtained first, then the summaries and observations commit atomically, so a count or bookkeeping failure records a failed run and leaves the prior current projection unchanged. It is deliberately absent from the everyday Mirror UI: ordinary push/pull operations establish observations, and the product does not ask users to manufacture history.
...
A compact mirror-only projection (`base_database_data_share_observations`) keyed by Local instance, remote instance, and table stores only the latest successful observations. The main Mirror table reads this projection on every render, independently of any remote catalog cache, so endpoint changes never display counts from a different mirror and history retention never removes the current state. Imports have no remote endpoint to compare, so they write ledger rows but not this endpoint-scoped projection.
...
1. **Keep one blocking combined catalog and rely on a per-session cache.** Rejected. The first visit remains slow, cache work is repeated across sessions, and a remote failure prevents Local tables from rendering even though Local owns selection.
...
- [ ] Add a shared endpoint-scoped stale-while-revalidate remote snapshot carrying an observation time and explicit invalidation when the configured endpoint changes. The session-only combined snapshot it replaces has already been removed.
...
- [x] Record nullable, timestamped effect counters and key ranges without overstating native count equality as transfer verification. {Amp/OpenAI Codex}
- [ ] Persist schema fingerprints from the exact review/execution observations; mirror summaries persist counters and ranges but still leave the fingerprint columns null.
...
- [x] Build the central `/admin/system/data-operations` history/detail surface and add persistent **Local rows**, **Remote rows**, **Observed**, and **Freshness** columns with completion deep links into the central surface. Fold presence into the two count cells instead of dedicating two redundant columns to Present badges. {Amp/OpenAI Codex; OpenAI Codex/GPT-5}
...
- [x] Retain an all-or-nothing, labelled retrospective-baseline backend primitive for exceptional recovery work, without exposing it as an ordinary user task or inventing a historical push. {Amp/OpenAI Codex; OpenAI Codex/GPT-5}
...
- [ ] Give `DataFreshnessTracker::compact()` a real caller. The race-safe compaction statement is implemented and tested, but nothing schedules or invokes it, so `base_database_data_freshness_events` grows one row per tracked statement once triggers are attached. Wire it to the scheduler (or the attachment command) before broad attachment.
