...
### Phase 4 — Catalog deferral simplification
...
- [ ] Inline the catalog as a lazy island fed by a parent computed; remove `CatalogBrowser`, placeholder, props.
- [ ] Initial HTML within budget; catalog still renders after first paint; tests green.
...
- [ ] Add "`@island` vs `#[Lazy]` child vs `wire:poll`" note (with data-isolation + anti-patterns) to `docs/guides/page-rendering-performance.md`.
- [ ] Cross-link this plan from `performance-page-rendering.md` (unused-leverage note + eBay Phase 3); mark addressed.
