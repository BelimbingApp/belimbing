...
Keep inspector behavior shared. Audit history and trace drawers use the shared `x-ui.inspector-drawer` shell so width, mask, mobile, and page-scroll behavior change in one place. On desktop it behaves as a non-modal inspector: no desktop mask, pointer/wheel passthrough outside the panel, and resizable width from a minimum readable column to the full viewport. On mobile it fills the viewport.
