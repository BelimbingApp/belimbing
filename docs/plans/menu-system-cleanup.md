...
- **Re-home Employee screens only.** `admin.employees` (and `admin.employee-types`) live under `admin` for historical reasons, not by design. Move the screens under a new `people` bucket while the canonical Employee module remains in Core because employee rows can belong to any company. People is the licensee-facing people lens; Addresses stays under `admin`, paired with Geonames as foundational reference data.
...
- [x] Create `app/Modules/People/Config/menu.php` declaring the `people` bucket (domain anchor file; Employee module stays under `Modules/Core/Employee/`) claude/opus-4-7
...
- [x] Re-parent `quality` (in `app/Modules/Operation/Quality/Config/menu.php`) and `it` (in `app/Modules/Operation/IT/Config/menu.php`) from `business` to `operations` claude/opus-4-7
...
- [x] `app/Modules/Operation/Quality/Config/menu.php` — `quality` → `operations.quality`; `quality.ncr` → `operations.quality.ncr`; `quality.scar` → `operations.quality.scar`
