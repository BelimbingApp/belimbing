...
| Payroll handoff | Approved reimbursable claim lines submit `PayrollContributionPayload`s to Payroll's `PayrollContributionIntake` with source reference, period anchor, pay item code, currency, amount, taxable metadata, and accounting snapshot. Payroll resolves the run, enforces duplicate protection via its composite unique index, and materialises a `PayrollInput::TYPE_REIMBURSEMENT` row (or holds the contribution pending). | Claim Core emits via intake; Payroll/country pack classifies |
...
- only approved, payroll-eligible claim lines submit contributions (source_type `claim_line`) through Payroll's `PayrollContributionIntake`;
- each payload carries `source_type`, `source_id`, `pay_item_code`, `period_anchor`, employee, label, approved amount, currency, occurred date, and metadata with claim request/line identifiers, policy version, route snapshot, and accounting mapping snapshot;
- duplicate handoff is rejected by Payroll's DB-level unique index `(source_type, source_id, pay_item_code, period_anchor)` on `people_payroll_pending_contributions` — Claim does not enforce uniqueness itself;
- claims in a locked payroll period are corrected through `PayrollContributionIntake::reverse(...)`; Payroll decides whether to delete the input (still in draft), insert a compensating reversal, or return `rejected_locked`;
- country packs decide statutory treatment of the materialised pay item; Claim Core does not compute PCB/EPF/SOCSO/EIS effects.
...
**Direction inverted (Phase 4 of plan 10):** `ClaimPayrollHandoffService` now writes through Payroll's `PayrollContributionIntake` contract. Claim no longer imports `PayrollInput`/`PayrollRun`/`PayrollRunParticipant`. Duplicate protection and locked-period correction are Payroll's responsibility; pending state (no open run) is durable in `people_payroll_pending_contributions` and materialises when a covering run opens.
