...
The existing People and Payroll plans cover module architecture, statutory calculation, outputs, employee self-service documents, and payroll-adjacent workflows, but the SBG iPayroll reference shows a large practical settings layer that is not yet planned as a first-class deliverable. That layer includes reference-data setup, organization taxonomy, employee workbench controls, employee account access provisioning, operational imports/logs, restricted-person handling, and SBG-specific seed/reference data.
...
| Employee workbench controls | Employee Master grid with filters, saved query, e-Letter, pay rate, cost center, hire/resign dates | Employee module is currently planned as a listing/profile, but not as an HR workbench with saved views and document actions. | Employees module, PDF/document services for letters |
| Employee account access provisioning | ESS Access grid with employee-to-user mapping, user ID, display name, email, activation, broadcast login info, batch update | Existing plans cover employee self-service features, but not identity provisioning, bulk activation, login notification, or access state management. BLB should not assume this requires a separate ESS module. | Existing authz/admin surfaces plus employee self-service People workflows |
| Employee request intake | Employee Request menu under Payroll | e-Request is listed in parity terms, but needs a controlled profile-change workflow tied to employee data ownership. | Employee self-service People workflows plus Employees approval workflow |
...
| Organization chart | Organisation Chart screenshot and HR2000 benchmark item | Organization chart is noted as later parity but lacks a dependency on clean organization reference data. | Reports/Employees after taxonomy is stable |
...
**Do not create a separate ESS module by default.** ESS means Employee Self Service, but in BLB the better product shape is employee self-service capabilities running through the normal People UI shell, authentication, and authorization model. The thing to build is explicit employee account access provisioning and controlled employee workflows, not a standalone legacy-style ESS island.

**Make employee account access provisioning explicit.** Employee self-service features are not just routes. Admins need to see which employees have user accounts, whether access is active, which email/login identifier is used, when login information was sent, and what batch changes were applied. The existing `EmployeePortalAccess` model name is acceptable as an implementation artifact for now, but product copy and docs should prefer Employee Account Access because the object being managed is an employee's account and access state.
...
- **Employee account access state:** employee-to-user mappings expose account status, login identifier, display name, email, activation state, last notification, batch operation audit, and access revocation history.
...
| Employee Master | Employee Workbench | "Master" is system-centric and imprecise. The screen is a working HR/payroll surface over employees. |
...
| ESS / Employee Self Service | Employee Self-Service Workflows | Do not create a separate module solely because iPayroll has ESS. BLB should expose employee actions through the existing People UI/authz model. |
...
- [x] Catalog every SBG iPayroll setup surface from `sbg_ipayroll_ref`, grouped by Core People, Payroll Core, Malaysia country pack, employee self-service workflows, platform admin, and SBG private customization. Implemented as the People Settings module with typed reference-data categories and migration source metadata. {GitHub Copilot/gpt-5.5}
...
- Should employee account login continue to mirror Employee No. for SBG, or should BLB require email/login identifiers independent of employee numbers?
