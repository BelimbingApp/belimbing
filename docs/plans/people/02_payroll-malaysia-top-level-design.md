...
**Status:** Phase 5 in progress — Malaysia contribution calculations are component/category-aware; PDF report templates and RenderPdfJob payload wiring started
...
- [ ] Produce payslip PDF for a closed payroll run via `RenderPdfJob` against `resources/core/views/pdf/payroll/payslip.blade.php`. The payslip Blade template now consumes the real `PayrollPayslipBuilder` data shape, and `PayrollPdfReportJobFactory::payslip()` creates the inline `RenderPdfJob` payload with template/data lineage metadata. Persisting the resulting `PdfArtifact` against a payslip/document record remains open.
