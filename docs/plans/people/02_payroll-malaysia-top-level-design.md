...
- [x] Implement Malaysia EPF, SOCSO, EIS, and HRD levy calculators using component-specific classified statutory wage bases and effective-dated rule-table rows before PCB. EPF/SOCSO/EIS emit employee and employer contribution lines; HRD emits employer levy lines when the employer statutory profile marks HRD levy applicable. Rule rows can now match employee category snapshots such as citizenship, age, and component-specific categories; missing required schedules block calculation only when an applicable wage base exists. {amp/gpt-5.1-codex, GitHub Copilot}
...
- [x] Complete calculation explanations across EPF/SOCSO/EIS/HRD at the current scaffold level: wage base, component wage-base keys, employee category snapshot, statutory version, rule set/row, rates or fixed amounts, rounding rule, profile IDs, pack, and share are recorded. Official table import and category semantics remain open research before production compliance claims. {amp/gpt-5.1-codex, GitHub Copilot}
- [x] Show statutory deductions/contributions on the payslip data shape and expose employer contribution/levy sections separately from employee take-home deductions. Add the first employer-cost report data builder over result lines. PDF/report templates and artifact persistence remain Phase 5 work. {GitHub Copilot}
...
- [x] Produce payslip PDF for a closed payroll run via `RenderPdfJob` against `resources/core/views/pdf/payroll/payslip.blade.php`. The payslip Blade template consumes the real `PayrollPayslipBuilder` data shape, `PayrollPdfReportJobFactory::payslip()` creates the inline `RenderPdfJob` payload with template/data lineage metadata, and `StorePayrollPdfArtifact` persists `PdfArtifactRendered` lineage into `payroll_pdf_artifacts` for payslips and run-level payroll PDFs. {GitHub Copilot}
- [x] Produce payroll summary, employee statutory contribution, and employer cost reports as their own Blade templates under `resources/core/views/pdf/payroll/`. Use `PdfRenderer::renderInline` for static reports (no auth dependency); `renderView` only where a report depends on per-user state. Baseline report builders and `PayrollPdfReportJobFactory` methods now create inline `RenderPdfJob` payloads for payroll summary, employee statutory contribution, and employer cost reports. {GitHub Copilot}
- [x] Generate the first SBG-needed bank payment export or a clearly marked bank-export placeholder while the exact bank format is confirmed. `PayrollBankPaymentExportBuilder` now emits an internal-review CSV marked `placeholder_not_bank_submittable`, reads optional employee `metadata.payroll_bank` details, flags missing bank details per row, and the Malaysia country pack advertises `bank_payment_placeholder` as a non-submittable CSV export. {GitHub Copilot}
- [x] Export operational reports to practical CSV/PDF formats. PDF goes through `RenderPdfJob`; `PayrollOperationalCsvExportBuilder` now emits CSV for payroll summary, statutory contribution, employer cost, and lock/audit reports from the same report-builder data used by the PDF templates. {GitHub Copilot}
- [ ] Add official statutory text-file emitters where applicable. CP39, EPF, SOCSO, EIS, and bank GIRO text files are plain-text emitters in the Malaysia country pack and do **not** touch the PDF pipeline; exact official layouts remain pending statutory/source-format confirmation.
- [x] Provide a payroll lock/audit report for review before and after closing a run. Baseline `PayrollLockAuditReportBuilder`, `pdf.payroll.lock-audit`, and `PayrollPdfReportJobFactory::lockAudit()` now summarize lifecycle timestamps, lock/review/approval state, result-line totals, participant totals, and audit events. {GitHub Copilot}
