...
| People employee UI | `people.employees.index` lists/searches employees in the current tenant's company tree. | It has no employee detail route, work-profile columns, filters by BLB classifications, saved views, bulk operations, or drill-down sections. |
...
**Make People/Employees the workbench, not Core/Admin.** Admin employee screens are for system operators. HR users need a People-facing workbench scoped to the current tenant's company tree, with operational columns, filters, saved views, and actions that reflect payroll and HR data readiness.
