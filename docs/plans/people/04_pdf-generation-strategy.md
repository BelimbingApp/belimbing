...
- [ ] Verify `qpdf` is available on every supported deployment target (FrankenPHP, Octane on Linux, Windows dev hosts) and that the specific operations the plan relies on — password protection, encryption, merging, and any proposed metadata mutation — are supported by the qpdf versions those targets ship. **Partial:** `qpdf` is NOT installed on the Windows dev host (verified `command not found`). FrankenPHP image and Linux Octane targets not yet checked. This row stays open pending an installer step in `docs/guides/` or a supported-targets narrowing — see Findings below.
...
- [ ] Measure renderer throughput and per-render memory under the AI browser tool’s existing workload, so contention and pooling assumptions are grounded in data and feed Phase 2’s concurrency design. **Pending:** requires spawning Chromium against the spike endpoint with real network/IO. Tests so far exercise the auth + artifact-write paths with a mocked `PlaywrightRunner`; the real-Chromium pass is a separate session.
