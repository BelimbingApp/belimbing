...
- `docs/plans/people/10_payroll-intake-dependency-inversion.md` - Payroll-owned intake and pending-store pattern that Attendance now publishes into through events.
- `docs/plans/people/11_attendance-shift-and-allowance-coverage.md` - multi-break shifts, shift-scoped allowances, and the roster/allowance shape that feeds the next attendance coverage slice.
- `docs/plans/people/12_attendance-event-decoupling.md` - Attendance now publishes payroll-facing events and Payroll listens/materializes them without Attendance importing Payroll classes.
- `docs/plans/people/13_leave-event-decoupling.md` - sibling event-seam pattern for Leave, useful as the second producer-side proof that the boundary holds.
- `docs/plans/people/14_claim-event-decoupling.md` - sibling event-seam pattern for Claim, including reverse-path fan-out.
- `docs/plans/people/15_attendance-roster-builder-ux.md` - roster builder UX slice that defines the current roster-first surface and its validation/publish contract.
- `docs/plans/people/16_attendance-roster-builder-dhh-followups.md` - DHH follow-ups for the roster surface: prose summary, prose filters, zoom levels, and calmer form composition.
- `docs/plans/people/16_leave-pay-item-mapping.md` - Payroll-side pay-item mapping pattern for Leave; useful precedent for keeping pay-item ownership out of source-domain rows.
- `docs/plans/people/17_claim-pay-item-mapping.md` - Payroll-side pay-item mapping pattern for Claim; same precedent as Leave.
- `docs/plans/people/18_roster_master.md` - master roster plan covering roster browse mode, self-service, lock/reconciliation, and the companion phase index.
- `docs/plans/people/18a_roster-surface-completion.md` - roster surface completion slice, including the remaining publish/validation surface cleanup.
...
BLB has a substantial Attendance module, but the remaining work is the operator-facing glue: workflow routing for exceptions, setup activation and review gates, allowance materialization, payroll reconciliation/reporting, and device/geofence/mobile parity. Without those pieces, attendance facts still need manual follow-through before they are fully explainable and payroll-ready.
...
| Attendance allowance rule | Typed conditional allowance definition with active state, ceiling, min/max resolution, daily/monthly condition rows, typed predicates, and source-script preservation for import/audit. Payroll pay-item mapping lives in Payroll-side lookup tables/listeners, not on the Attendance rule itself. | Attendance Core; private packs seed formulas |
| Guided setup operations | Deterministic draft, validate, simulate, import, explain, roster-draft, roster-validate, and roster-publish operations exposed to the UI and optional Artisan/operator commands. Explicit activation/review-gate flows remain planned. These operations are the People-domain provision; Lara, skills, or other AI orchestration may consume them later but are not part of Attendance Core. | Attendance Core services; UI; Artisan/operator commands |
| Payroll handoff | Converts approved/finalized attendance facts into payroll-facing events and neutral contributions for overtime, attendance allowance, shift allowance, lateness deduction, unpaid absence, rest-day work, and one-off time adjustments. Attendance publishes producer-domain events today (`AttendanceOvertimeApproved`), and Payroll listeners materialize them through Payroll-owned intake/status APIs. Attendance never imports `PayrollInput`/`PayrollRun`/`PayrollRunParticipant` or any other Payroll model. | Attendance emits via event seam; Payroll/country pack classifies |
...
- neutral code, display name, active state, source alias, and optional accounting/export metadata;
...
- explanation output showing which rows matched, which rows were capped, and which Payroll-side mapping would be used when the allowance is materialized.
...
- [x] Wire Attendance to `EmployeeWorkProfile` and People Settings calendar references for employee calendar, supervisor, cost center, workforce class, and pay basis resolution. `AttendanceCalendarResolver` now reads `EmployeeWorkProfile` and `PeopleCalendarException` directly, and roster/day resolution consumes those references. {codex/gpt-5, claude-code/opus-4.7}
- [x] Consume `PeopleCalendarException` for company holidays via the employee's work calendar. `AttendanceCalendarResolver` reads exceptions with `kind` of `public_holiday` or `non_working_day` and reports them as `day_type = holiday`. Special-workday and employee-specific override kinds are out of scope until the calendar-exception taxonomy expands. {claude-code/opus-4.7}
- [x] Implement fixed weekly roster, rotating roster cycle, ad hoc roster rows, roster publishing, and revision history. The roster workbench now covers draft/published assignments, copy-previous-period, bulk cell overrides, swap/override flows, spreadsheet import, lock/unlock, acknowledgements, and roster-cell history. {codex/gpt-5, claude-code/opus-4.7}
- [ ] Implement typed rule-policy evaluation for grace, daily/monthly rounding, paid/unpaid breaks, pay-basis-specific break exclusion, break lateness, missing punch, late, early-out, absent, cross-midnight, daily-rated workday counting, and payroll attribution. `AttendanceDayProjectionService` now honors lateness grace, work-hour rounding, lateness rounding, per-break paid/unpaid deduction, OT minimum threshold suppression, missing-punch/late/early-out/absent tags, and cross-midnight worked-minute computation. `AttendanceDayResolverService` now honors `shift.payroll_attribution` — `shift_end_date` rolls the payroll period forward by one day for cross-midnight shifts. Also fixed a same-day boundary bug in `assignmentFor` / `shiftByCode` / `AttendancePolicyGroupResolver` where SQLite string-compared a stored `YYYY-MM-DD HH:MM:SS` against a `YYYY-MM-DD` argument, causing same-day resolutions to miss the assignment; switched to `whereDate` for proper date comparison. Remaining: monthly rounding (payroll-period concern), pay-basis-specific break exclusion + daily-rated workday counting, break-lateness offset (`less_break_lateness`). {codex/gpt-5, claude-code/opus-4.7}
- [x] Implement attendance policy group assignment so employees/cohorts resolve shift, work-hour, lateness, overtime, export, and allowance rules from one versioned bundle. {codex/gpt-5, claude-code/opus-4.7}
...
- [x] Generate neutral payroll contributions for approved overtime with overtime-request source references. `AttendanceOvertimeService::queuePayrollHandoff` now emits `AttendanceOvertimeApproved`, and Payroll's `RecordAttendanceOvertimeContribution` listener materializes the contribution through `PayrollContributionIntake`. The producer-owned `people_attendance_payroll_handoffs` table and `AttendancePayrollHandoff` model were retired; Payroll's `people_payroll_pending_contributions` now owns durable handoff state, including the "no open run yet" pending case. Source type: `'attendance_overtime_request'`. See `docs/plans/people/12_attendance-event-decoupling.md`. {codex/gpt-5,amp/claude-opus-4-7}
...
**Immediate slice:** source-evidence + activation flow, roster-change notifications, payroll tabs, reconciliation reports, CSV exports, and saved filters. Device/geofence and SBG import remain later phases.

- [x] Employee tabs in `people.attendance.index`: clock in/out where enabled, my schedule, my attendance, missing-punch/adjustment request, overtime request, and history. `MyAttendance` now covers clocking, attendance history, overtime requests, and adjustment requests on the employee surface. {codex/gpt-5}
- [x] Supervisor tabs gated by approval capability: subordinate attendance exceptions, overtime approvals, roster view, overlap/coverage warnings, and subordinate summaries. The approvals surface handles overtime and adjustment queues; the roster surface carries the schedule/coverage workbench for approvers. {codex/gpt-5}
- [x] HR/admin tabs: shift templates, attendance policy groups, roster patterns, roster assignment, rule policies, allowance rules, absenteeism batches, imports, manual corrections, finalization, and audit. The current workspaces already expose the setup, operations, roster, approvals, and locations surfaces that make up this tab set. {codex/gpt-5}
- [x] HR/admin guided setup: template-based Attendance Policy Group wizard, visual shift timeline editor, plain-language rule cards, validation findings, and simulation preview. {codex/gpt-5}
- [ ] Source-evidence panel and explicit version activation flow for saved setup drafts.
- [x] Promote Attendance setup out of generic Settings into first-class Attendance navigation: an Attendance Setup menu group now exposes Policy Groups (list + inline builder), Policy Validator, Shifts (shift template list + inline builder), and Allowance Rules; Roster Builder and Clocking Locations are direct Attendance workspaces. Originally shipped as "Policy Studio" with separate Library/Builder screens; consolidated to honest names and inline list+form pages in a later honesty-pass. {amp/gpt-5.5-medium}
- [x] Ship JSON policy-template exchange from the Policy Groups screen: users can start from built-in templates, import a template object/array into the builder, and export a builder draft or saved policy group as `belimbing.attendance.policy-template.v1` JSON for future country-pack/private-pack/template-repo reuse. {amp/gpt-5.5-medium}
- [x] Supervisor roster builder: team/date-range calendar, coverage view, copy previous period, rotating pattern fill, bulk assignment, swap/override flow, validation warnings, publish preview, and revision history. See `15_attendance-roster-builder-ux.md` for the focused UX/build sheet.
- [ ] Employee notification intent from roster publish/changes. Publish intent is already recorded; the remaining gap is change-intent from roster edits and publish-adjacent updates.
- [x] Day-type-aware resolver: `AttendanceCalendarResolver` derives `day_type` ∈ {normal, rest, off, holiday} from the employee's work calendar (holidays from `PeopleCalendarException`; weekly rest/off from the calendar's metadata `rest_days` / `off_days` arrays). `AttendanceDayResolverService` consumes it and persists the correct `day_type` on every attendance day. Constants `AttendanceDay::DAY_TYPE_*` defined. {claude-code/opus-4.7}
- [x] `RosterPattern` definition supports a `day_types` map alongside `weekdays`. The resolver consults `day_types[$dayType]` first; a `shift_code: null` entry means "no shift on this day type." E.g. a `fixed_weekly` pattern can say "Thursday → PROD_DAY, but if it's a holiday → PROD_HOLIDAY_HALF, and Sunday rest day → no shift." Backwards-compatible: patterns without a `day_types` map still route purely by weekday/cycle offset. {claude-code/opus-4.7}
- [ ] Bulk roster reassignment by employee attribute, e.g. religion = Islam for Ramadan effective range. The natural BLB pattern for date-range shift variations (Ramadan, festive half-days, plant-shutdown rotations) is a separate shift template plus roster reassignment across the relevant employees; the friction is selecting that population in bulk. See `11_attendance-shift-and-allowance-coverage.md` for the framing. This is a later roster-ops follow-up, not part of the immediate setup/reconciliation slice.
- [x] Add optional Artisan commands for roster draft, validate, explain, and publish dry-run, emitting stable JSON for batch/operator workflows. `RosterCommand` now emits stable JSON for those operator flows. {codex/gpt-5}
- [ ] Payroll tabs: attendance export queue, payroll handoff status, reconciliation, reversal/correction review, and lock-state warnings. This is the payroll-facing part of the immediate slice.
- [ ] Reports as `RenderPdfJob` outputs under `resources/core/views/pdf/attendance/`: attendance summary, overtime list, late/absence exceptions, absenteeism batch record, roster, payroll reconciliation, and monthly attendance statement. Keep this aligned with the payroll reconciliation slice.
- [ ] CSV exports for attendance days, clock events audit, overtime, roster, exceptions, absenteeism batches, geofence audit, and payroll handoff. This should ride with the reporting/reconciliation surface, not wait for the later device work.
- [ ] Saved filters/query views for timecard, absenteeism, overtime, clock transaction, and payroll reconciliation grids. These are the final operator affordances for the immediate slice.
