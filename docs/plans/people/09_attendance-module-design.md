...
- [ ] Day-type-aware resolver: `AttendanceDayResolverService::resolve()` currently sets `day_type` purely from "has a shift = normal, else off". Consult `PeopleCalendarException` for public holidays and the employee's calendar for rest/off days so the persisted `day_type` reflects reality. Without this, downstream OT-on-holiday / rest-day-rate-scaling rules cannot fire correctly.
- [ ] `RosterPattern` definition extended to route by day type, not only day-of-week and cycle offset. E.g. a `fixed_weekly` pattern may say "Monday → PROD-DAY, but if Monday is a public holiday → PROD-HOLIDAY-HALF". Schema: add an optional `day_types` map alongside the existing `weekdays` map; resolver consults `day_types` first.
