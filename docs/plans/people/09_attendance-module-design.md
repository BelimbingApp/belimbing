...
- [ ] Implement attendance-day projection from roster, rule policy, clock events, and calendar exceptions while the period is open. Roster-aware day resolution, clock-event projection, and calendar-exception integration are implemented (day_type now derived from `PeopleCalendarException` + work-calendar metadata); rule-policy projection (lateness, rounding, break deduction, OT bands) remains open. {codex/gpt-5, claude-code/opus-4.7}
...
- [ ] Supervisor roster builder: team/date-range calendar, coverage view, copy previous period, rotating pattern fill, bulk assignment, swap/override flow, validation warnings, publish preview, revision history, and employee notification intent. See `15_attendance-roster-builder-ux.md` for the focused UX/build sheet.
