...
| Payroll handoff | Unpaid leave days and any encashment lines surface as neutral `PayrollInput` rows for the next draft run; the Malaysia country pack classifies them for EPF/SOCSO/EIS/PCB treatment. **Direction inverting:** `LeavePayrollHandoffService` and `LeaveEncashmentService` will be ported to call `PayrollContributionIntake` per `docs/plans/people/10_payroll-intake-dependency-inversion.md`. New handoff code in Leave should not import `PayrollInput`/`PayrollRun`/`PayrollRunParticipant`. | Leave emits via Payroll intake; Payroll/country pack classifies |
...
- **Risk: leave-to-payroll double-counting.** Guardrail: each unpaid-leave or encashment `PayrollInput` row carries the leave request ID; Payroll inputs reject duplicates with the same source reference within the same period. Schema-level uniqueness and the move of handoff ownership into Payroll are tracked under `docs/plans/people/10_payroll-intake-dependency-inversion.md`; Leave's current `LeavePayrollHandoffService` is the producer-side path that plan retires.
