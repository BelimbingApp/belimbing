...
**Agents:** claude/opus-4.8 (identification only)
...
- **Separate stale from correct usage.** Of the ~36 files referencing `payroll_pay_item_code`, most are legitimate Payroll mapping usage (the mapping models, the `*PayItemMapping` Livewire components/views, the mapping migrations). Only the references that **assign or read it on `AttendanceAllowanceRule` / `ClaimType` (or their views)** are stale and in scope here. Known suspects to verify first: `ClaimAccountingExportBuilder`, `ClaimOperationsExportBuilder`, the attendance allowance-rule create path, and tests `AttendancePolicyOperationsTest` / `ClaimAccountingExportTest`.
- **Preserve export output.** The accounting/operations exports exist to emit pay-item codes; redirect them to the mapping tables rather than dropping the feature — confirm the exported values are unchanged for a representative fixture.
...
- [ ] From the `payroll_pay_item_code` usages, list exactly which sites assign/read it on `AttendanceAllowanceRule` / `ClaimType` (or render `$rule->payroll_pay_item_code` / `$claimType->payroll_pay_item_code` in a view), separating them from the legitimate mapping-table usage.

### Phase 2 — Redirect the callers

- [ ] Update each stale caller to resolve the pay-item code via `PayrollAttendanceRulePayItem` / `PayrollClaimTypePayItem` (by rule / claim-type id), preserving current export output.
- [ ] Update the affected tests/factories to seed the mapping rows instead of assigning the dropped column.

### Phase 3 — Enable the guardrail

- [ ] Enable `preventSilentlyDiscardingAttributes` in `AppServiceProvider::configureModels()` (alongside the already-enabled `preventLazyLoading` + `preventAccessingMissingAttributes`).
- [ ] Run the full suite with the project ini (`PHPRC` → sodium): confirm green and zero `MassAssignmentException`.
