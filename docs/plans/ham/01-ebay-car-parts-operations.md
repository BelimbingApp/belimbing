...
3. **Marketplace** (`app/Modules/Commerce/Marketplace` + channel providers) — channel host + adapter registry. Read-first: pulls Ham's existing eBay listings and orders so BLB has a populated ledger from day one without him doing data entry. The first framework slice has landed with a registry-driven host, eBay as the first registered channel provider, eBay OAuth connection surface backed by `base_settings` and encrypted OAuth token storage, Inventory API listing/offer sync that materializes `Listing` rows and links by SKU when possible, Fulfillment API order sync into the Sales ledger, and a browser reconciliation view over synced listings vs. local inventory. Shopee/Lazada can be added through the same channel-provider registration path without changing the host module.
4. **Sales** (`app/Modules/Commerce/Sales`) — `Order`, `OrderLine`, `Sale` (the durable "this item was sold for this much on this date through this channel" record), `Fee`. Driven by Marketplace adapters. The first schema slice has landed with orders, lines, durable sales rows, item/listing links, and cost-basis snapshot from linked inventory.
...
- [ ] **Media subsystem** — storage abstraction, image processing queue, signed URLs, **HTTP upload endpoint**, **derived-asset model** (cleaned image is a child of the original, never an overwrite).
  - [x] **Slice 1 — `app/Base/Media`:** `MediaAsset` model + `base_media_assets` table with self-FK derivatives + open-vocabulary `kind`.
  - [x] **Slice 2 — Inventory photos:** `commerce_inventory_item_photos` on `MediaAsset`; pivot owns `item_id` / `media_asset_id` / `sort_order` only; file metadata (disk, `storage_key`, `original_filename`, `mime_type`, `file_size`) lives on the asset.
  - [x] **Slice 3 — `MediaAssetStore`:** `putUploadedFile()`, `putDerivativeBytes()`, and `delete()` persist/remove bytes via Laravel filesystems with row registration; consumers no longer use the `Storage` facade for media. Low-level `storeOriginal()` / `storeDerivative()` remain when bytes are already on disk (e.g., Quality `EvidenceService`). Phase 4 `photo-cleanup` can register a cleaned child with `putDerivativeBytes($original, 'background_removed', ...)`.
  - [x] **Slice 4 — Signed URLs:** `MediaAssetController::stream` at `media/assets/{asset}/stream`, Laravel `signed` middleware, `MediaAsset::streamUrl()` and `MediaAssetStore::temporaryStreamUrl()` (short-lived URLs, default 5 min).
  - [x] **Slice 5 — Generic stream route:** Retired bespoke `ItemPhotoController`; Inventory item detail emits `$asset->streamUrl()` and the browser hits the generic media stream route.
  - [x] **Slice 6 — Delete cascade:** `MediaAssetStore::delete()` walks the derivative tree and removes every file, not only the parent — closes orphaned-derivative-files risk before Phase 4.
  - [x] **Slice 7 — Quality evidence:** `Quality\EvidenceService` uses `putUploadedFile()` for `upload()` / `replace()`; `EvidenceStorageException` replaced by `MediaStorageException`.
  - [ ] Image processing queue.
  - [ ] Generic HTTP upload endpoint.
...
- [x] `app/Modules/Commerce/Inventory`: add `ItemPhoto` and the photo grid/upload surface. `ItemPhoto` is now a thin pivot owning `item_id` / `media_asset_id` / `sort_order`; file metadata lives on the linked `MediaAsset` so Phase 4 cleanup can register derived assets without overwriting originals. Image processing queue, signed URLs, and a generic HTTP upload endpoint remain on the Phase 0 media-subsystem item.
...
- [x] `app/Modules/Commerce/Sales`: first schema slice with `Order`, `OrderLine`, and `Sale`; `Sale` is the durable record of "this sold."
- [x] Materialize pulled eBay orders as `Order`/`OrderLine`/`Sale` rows, auto-link by listing/SKU when present, and mark linked inventory items sold.
- [x] Reconciliation view: BLB inventory vs. live eBay listings (what's listed, what isn't, what's drifted). First UI slice landed on the eBay Marketplace page with listing/order pull actions, matched/unlinked/drifted listing badges, and active BLB inventory missing from eBay.
...
- [x] General Insights query surface in `app/Modules/Commerce/Sales` — single-currency, single-company read methods used by all Ham layouts and any future tenant. Sub-slices:
    - [x] `SalesInsightsService::soldInPeriod($companyId, $from, $to, $currencyCode)` — returns a `SalesPeriodSummary` DTO (sale count, unit count, total revenue/cost/fees in minor units, computed gross profit) for a single-currency window scoped to one company.
    - [x] `SalesInsightsService::marginPerItem(..., ?$limit)` — returns a Collection of `ItemMarginRow` DTOs grouped by `item_id`, ordered by gross profit DESC in SQL, with optional top-N limit; unmatched sales (null `item_id`) are surfaced as their own row so revenue gaps are visible.
    - [x] `SalesInsightsService::daysListedWithoutSale($companyId, $currencyCode, ?$asOf, ?$minDaysListed, ?$limit)` — returns a Collection of `AgedListingRow` DTOs covering active listings (`ended_at IS NULL`, `listed_at NOT NULL`) with no associated `Sale` row, ordered oldest-listed first; day deltas are computed in PHP for driver-agnostic SQL while `minDaysListed` translates to a `listed_at` upper bound so filtering happens in the database.
    - [x] `SalesInsightsService::salesInPeriod(..., ?$limit)` — returns a Collection of `RecentSaleRow` DTOs (newest first) with title resolved at query time (prefer `Item->title`, fall back to `OrderLine->title`, then channel SKU); eager-loads `item.category` and `orderLine` to avoid N+1.
    - [x] `SalesInsightsService::salesByCategory(..., ?$limit)` — returns a Collection of `CategorySalesRow` DTOs grouped by category, ordered by `COUNT(*) DESC`; left-joins through `commerce_inventory_items` so sales without a linked item, or with an item that has no `category_id`, collapse into a single null-id "Uncategorized" bucket.
- [ ] Ham extension Livewire pages — thin views over the general queries, shaped to Ham's *used-parts* inventory. Because every salvage-grade item is essentially one-off (one specific 2008 Civic headlight, one specific salvaged bumper), Ham's actionable signals are recency, frequency, and grouping by *catalog dimensions* (category, make/model/year), not rank-by-`item_id`. The **"Top earners last 90 days"** layout is kept as a harmless framework demo — useful to other future tenants whose SKUs repeat — but isn't load-bearing for Ham. Sub-slices:
    - [x] `Extensions\Ham\AutoParts\Livewire\Insights\SoldThisMonth` — calls `SalesInsightsService::soldInPeriod` over the current calendar month and renders a six-tile card (sales, units, revenue, cost, fees, gross profit). Route gated by `commerce.marketplace.list` (temporary fit until a `commerce.sales.view` capability lands). The extension's `ServiceProvider` now registers a `ham-auto-parts` view namespace via `loadViewsFrom`, so templates ship under `extensions/ham/auto-parts/Views/` regardless of `VITE_THEME_DIR`. A companion `DevSaleSeeder` (depending on `DevInventoryItemSeeder`) seeds seven idempotent demo `Order`/`OrderLine`/`Sale` rows across the current and prior two months so dev environments render real numbers.
    - [x] `Extensions\Ham\AutoParts\Livewire\Insights\TopEarnersLast90Days` — calls `SalesInsightsService::marginPerItem` over `now-90d → now` with a top-10 limit, bulk-loads item titles, renders a ranked table; null-`itemId` rows surface as "Unmatched listings." Kept as a generic framework demo even though it's a poor layout for used-parts inventory specifically.
    - [x] `Extensions\Ham\AutoParts\Livewire\Insights\RecentSales` — operator-configurable from/to date range (default last 30 days, persisted via Livewire `#[Url]` so the URL reflects the filter); capped at 100 rows with a "narrow the date range" hint when the cap is hit. Invalid range (start > end) renders an alert instead of swallowing/swapping.
    - [x] `Extensions\Ham\AutoParts\Livewire\Insights\SalesByCategory` — same configurable from/to UX (default last 90 days), capped at 50 rows. Sales whose linked item has no category, and unlinked sales, collapse into a single italic-muted "Uncategorized" bucket.
    - [x] `Extensions\Ham\AutoParts\Livewire\Insights\ListedWithoutSale` — uses `SalesInsightsService::daysListedWithoutSale` with a default `minDaysListed = 180` (operator-configurable, persisted via Livewire `#[Url]`); ranked oldest-listed first; capped at 100 rows. A companion `DevListingSeeder` (in the Marketplace module, depending on `DevInventoryItemSeeder`) seeds five idempotent demo listings spread across `60d–245d` ago so the page has visible data in the licensee dev environment.

    All Ham insight views follow the project's UI reference catalog (`admin/system/ui-reference/*`): `x-ui.input` with `:label`, `x-ui.datetime`, `x-ui.badge`, `x-ui.alert`, the dashed-border-with-icon empty state, and the standard table classes with `px-table-cell-x` / `py-table-cell-y` / `tabular-nums` rhythm.
- [x] CSV export for Ham's bookkeeper. `App\Modules\Commerce\Sales\Http\Controllers\SalesCsvExportController` (single-action, registered at `commerce/sales/export.csv` and named `commerce.sales.export.csv`) streams a `StreamedResponse` of `Sale` rows for the authenticated operator's company, scoped by `from`/`to`/`currency_code` query params (defaults to last 30 days in the company's default currency). Iterates with `chunk(500)` so the export scales past the in-memory cap. CSV columns: `sold_at` (ISO 8601), `channel`, `external_sale_id`, `item_id`, `sku`, resolved `title` (Item → OrderLine → SKU fallback), `category`, `quantity`, `sale_amount`, `cost_basis_amount`, `fee_amount`, `gross_profit`, `currency_code` — money fields rendered as decimal strings via `Money::formatInput()` so the receiving spreadsheet doesn't need to know minor-units. Surfaced from the Recent sales page via a "Download CSV" header button that mirrors the operator's current `from`/`to` filter, gated by `commerce.marketplace.list`.
