...
- [ ] Ham extension Livewire pages — thin views over the general queries, shaped to Ham's *used-parts* inventory. Because every salvage-grade item is essentially one-off (one specific 2008 Civic headlight, one specific salvaged bumper), Ham's actionable signals are recency, frequency, and grouping by *catalog dimensions* (category, make/model/year), not rank-by-`item_id`. Target pages: **"Sold this month"** (aggregate), **"Recent sales"** (operator-configurable date range), **"Sales by category"** (frequency by `Category` over a window), **"Listed > 180 days, no sale"** (aging). The **"Top earners last 90 days"** layout is left in place as a harmless framework demo — useful to other future tenants whose SKUs repeat — but isn't load-bearing for Ham. Slices landed:
    ...
    - `Extensions\Ham\AutoParts\Livewire\Insights\RecentSales` — operator-configurable from/to date range (default last 30 days, persisted via Livewire `#[Url]` so the URL reflects the filter), backed by a new framework query `SalesInsightsService::salesInPeriod()` returning a `Collection<RecentSaleRow>` (sale id, sold-at, item id, resolved title — preferring `Item->title`, falling back to `OrderLine->title`, then channel SKU — category name, channel, quantity, sale amount). Capped at 100 rows with a "narrow the date range" hint when the cap is hit. Invalid range (start > end) renders an alert instead of swallowing/swapping. "Sales by category" and "Listed > 180 days, no sale" remain.
