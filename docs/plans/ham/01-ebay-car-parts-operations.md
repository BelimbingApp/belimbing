...
- `Inventory` primitives: `Item`, `ItemPhoto`, lifecycle states, internal SKU. Sales-by-construction — this module's `commerce_inventory_items` table only ever holds sellable items; non-sales domains (maintenance MRO, production raw materials) get their own modules with their own schemas, not rows in this table. *No* bin/location tracking — Ham doesn't need it and the next adopter can extend if they do.
...
- **`base_settings` UX + storage**: operator-editable provider API keys, OAuth credentials/tokens, channel bindings, provider plan/cap metadata, company default currency, and extension defaults. Ham must not need to edit `.env`.
...
- An `Item` has a stable BLB-internal SKU independent of any eBay item ID. Sold items retain their record forever — the **Sale** ledger is append-only and survives listing deletion on eBay.
...
- Provider credentials, eBay OAuth details/tokens, channel bindings, plan/cap metadata, and the company default currency are stored through `base_settings` and edited through BLB UX; Ham-specific extension files may provide defaults but never require `.env` edits or committed secrets.
...
- [ ] Materialize pulled eBay listings as `Listing` rows linked to `Item` (auto-link by SKU when present, operator-confirmed otherwise).
