...
- [ ] Media subsystem: storage abstraction, image processing queue, signed URLs, **HTTP upload endpoint**, **derived-asset model** (cleaned image is a child of the original, never an overwrite). Two slices have landed: (1) `app/Base/Media` framework — `MediaAsset` model + `base_media_assets` table with self-FK derivatives + open-vocabulary `kind`, plus a `MediaAssetStore` service for original/derivative row registration; (2) `commerce_inventory_item_photos` migrated onto `MediaAsset` — the pivot now stores only `item_id`, `media_asset_id`, `sort_order`, and the underlying file metadata (disk, storage_key, original_filename, mime_type, file_size) lives on `MediaAsset`. Phase 4 `photo-cleanup` can now attach cleaned children via `storeDerivative()`. Image processing queue, signed URLs, and a generic HTTP upload endpoint remain.
