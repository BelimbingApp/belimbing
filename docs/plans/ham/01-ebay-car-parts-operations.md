...
- [ ] Media subsystem: storage abstraction, image processing queue, signed URLs, **HTTP upload endpoint**, **derived-asset model** (cleaned image is a child of the original, never an overwrite). Three slices have landed: (1) `app/Base/Media` framework — `MediaAsset` model + `base_media_assets` table with self-FK derivatives + open-vocabulary `kind`; (2) `commerce_inventory_item_photos` migrated onto `MediaAsset` so the pivot only owns `item_id` / `media_asset_id` / `sort_order` and file metadata (disk, storage_key, original_filename, mime_type, file_size) lives on the asset; (3) `MediaAssetStore` now owns file persistence — `putUploadedFile()`, `putDerivativeBytes()`, and `delete()` write/remove bytes via Laravel filesystems and pair them with row registration, so consumers no longer touch the `Storage` facade for media. Low-level `storeOriginal()` / `storeDerivative()` remain for callers whose bytes are already on disk (e.g., Quality `EvidenceService`). Phase 4 `photo-cleanup` can now write a cleaned child with one call: `putDerivativeBytes($original, 'background_removed', ...)`. A fourth slice landed: signed-URL serving via `MediaAssetController::stream` at `media/assets/{asset}/stream`, gated by Laravel's `signed` middleware, plus `MediaAsset::streamUrl()` (and `MediaAssetStore::temporaryStreamUrl()` delegating to it) which mint short-lived signed URLs (default 5 min). A fifth slice retired the bespoke `ItemPhotoController` — the Inventory item detail view now emits `$asset->streamUrl()` and the browser fetches photos directly from the generic media stream route. A sixth slice tightened `MediaAssetStore::delete()` to walk the derivative tree and clean up every file, not just the parent's — closing the orphaned-derivative-files gap before Phase 4 starts producing them. A seventh slice migrated `Quality\EvidenceService` (the second module consumer) to the high-level API: `upload()` and `replace()` now route through `putUploadedFile()`, and the bespoke `EvidenceStorageException` is gone in favour of the framework's `MediaStorageException`. Image processing queue and a generic HTTP upload endpoint remain.
...
- [ ] General Insights query surface in `app/Modules/Commerce/Sales`: sold-in-period, gross margin per item, days-listed-without-sale. Two slices landed: (1) `SalesInsightsService::soldInPeriod($companyId, $from, $to, $currencyCode)` returns a `SalesPeriodSummary` DTO (sale count, unit count, total revenue/cost/fees in minor units, computed gross profit) for a single-currency window scoped to one company. (2) `SalesInsightsService::marginPerItem(..., ?$limit)` returns a Collection of `ItemMarginRow` DTOs grouped by `item_id`, ordered by gross profit DESC in SQL, with optional top-N limit; unmatched sales (null item_id) are surfaced as their own row so revenue gaps are visible. Days-listed-without-sale remains.
