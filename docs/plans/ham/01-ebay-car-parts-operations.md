...
- `Inventory` primitives: `Part`, `PartPhoto`, lifecycle states, internal SKU. Sales-by-construction — this module's `commerce_inventory_parts` table only ever holds sellable items; non-sales domains (maintenance MRO, production raw materials) get their own modules with their own schemas, not rows in this table. *No* bin/location tracking — Ham doesn't need it and the next adopter can extend if they do.
...
- `Marketplace` contract: an interface `MarketplaceChannel` with `pullListings()`, `pullOrders()`, `createListing()`, `reviseListing()`, `endListing()`. Each adapter is a separate module under `app/Modules/Commerce/Marketplace/{Channel}` so unused channels are not loaded.
- `Marketplace/Ebay` adapter (general): OAuth, listing + order sync (Phase 2), then researched publish/revise/end via the Sell APIs (Phase 6).
- `Core/AI` extensions: two new **Lara tasks** (per `lara-task-models.md`):
  ...
  - `describe-part` (simple task) — photos + attributes → title/description/category drafts.
...
- Cross-cutting BLB primitives: `Money` value object, file/media subsystem with mobile-friendly upload, `base_settings` UX/storage for operator-editable provider/channel/account settings, external HTTP/OAuth/webhook primitive — these are framework gaps regardless of Ham.
...
- Ham's preferred description template / tone / boilerplate footers, and his car-parts-tuned `photo-cleanup` / `describe-part` prompt overrides.
...
1. **Inventory** (`app/Modules/Commerce/Inventory`) — `Part`, `PartPhoto`, lifecycle (`draft → ready → listed → sold → archived`), internal SKU. Sales-by-construction: this module's tables only hold sellable items. A future maintenance or raw-materials domain is a separate module, not a flag in this one. Deliberately thin: no bins, no movements ledger, no warehouse model. The first slice has landed with `Part`, `commerce_inventory_parts`, internal SKU generation, and a browser-visible Parts Workbench.
2. **Catalog** (`app/Modules/Commerce/Catalog`) — `ProductTemplate`, `Category`, `Attribute`, `AttributeValue`, `Description` (versioned). The mechanism for "what is this sellable part"; the car-parts vocabulary is seeded by Ham's extension. Maintenance categories or BOMs belong in their own module's catalog, not here.
...
4. **Sales** (`app/Modules/Commerce/Sales`) — `Order`, `OrderLine`, `Sale` (the durable "this part was sold for this much on this date through this channel" record), `Fee`. Driven by Marketplace adapters.
5. **AI Assist** — extends `app/Modules/Core/AI` with two new Lara tasks: `photo-cleanup` (raw image → background-removed image) and `describe-part` (photos + attributes → title/description/category drafts). Both flow through the existing Lara per-task model selection, provider-side plan/cap controls where available, and BLB usage visibility.
...
- **`Money` value object** + currency-aware columns; no floats for cash, ever.
- **`base_settings` UX + storage**: operator-editable provider API keys, OAuth credentials/tokens, channel bindings, plan/cap metadata, and extension defaults. Ham must not need to edit `.env`.
- **External HTTP / OAuth primitive**: typed clients, token storage, retry + rate-limit policy, webhook receiver. Needed by the eBay adapter and any future channel.
...
- **Money as integer-cents + currency code,** wrapped in a `Money` value object. Cheap now, painful later.
...
- A `Part` has a stable BLB-internal SKU independent of any eBay item ID. Sold parts retain their record forever — the **Sale** ledger is append-only and survives listing deletion on eBay.
- The `MarketplaceChannel` contract guarantees `pullOrders()` is idempotent and produces `Sale` rows linked to the originating `Part` whenever a match is possible (by SKU, eBay item ID, or operator-confirmed link).
...
- The `describe-part` Lara task is a pure function from (`Part`, `ProductTemplate`, photo set, model choice, budget) to (`title draft`, `description draft`, `category suggestion`, `token cost`). It never auto-publishes.
- Provider credentials, eBay OAuth details/tokens, channel bindings, and plan/cap metadata are stored through `base_settings` and edited through BLB UX; Ham-specific extension files may provide defaults but never require `.env` edits or committed secrets.
- The Insights query surface (general) exposes: parts sold in `[from, to]` per channel/category, gross margin per part = sale − fees − cost basis, days-listed-without-sale per part. Extensions render these; they don't reimplement them.
...
- [ ] `Money` value object + currency-aware column convention; document under `docs/architecture/`.
- [ ] `base_settings` UX/storage for operator-editable API keys, OAuth details/tokens, channel bindings, provider plan/cap metadata, and extension defaults; Ham must never need `.env`.
...
- [x] `app/Modules/Commerce/Inventory`: first `Part` slice — `commerce_inventory_parts`, lifecycle status field, internal SKU generator, company scoping, list/create Workbench, and no location/bin/movement tables.
- [ ] `app/Modules/Commerce/Inventory`: add `PartPhoto` and the photo grid/upload surface.
...
- [x] Operator UX first slice (desktop-first): Parts Workbench lists parts, filters by search, and creates the durable part record with cost/list price/status/notes.
...
- [ ] `app/Modules/Commerce/Marketplace` contract.
- [ ] `app/Modules/Commerce/Marketplace/Ebay` adapter: OAuth (sandbox + live), token refresh, scheduled listing pull, scheduled order pull.
- [ ] Materialize pulled eBay listings as `Listing` rows linked to `Part` (auto-link by SKU when present, operator-confirmed otherwise).
...
- [ ] Ham's description boilerplate (footer text, return-policy blurb) and car-parts-tuned prompt overrides for `describe-part`.
...
- [ ] Register `photo-cleanup` and `describe-part` as new Lara simple tasks (per `lara-task-models.md`); they appear on the Task Models page alongside `titling`, `research`, `coding`.
...
- [ ] `describe-part` execution: photos + attributes → draft title, draft description, suggested category.
...
- [ ] Ham extension: car-parts-tuned `describe-part` prompt (fitment table, condition disclosure, OEM/interchange callout) and a sensible default `photo-cleanup` model pick.
...
- [ ] General Insights query surface in `app/Modules/Commerce/Sales`: sold-in-period, gross margin per part, days-listed-without-sale.
