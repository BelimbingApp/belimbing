...
- [ ] Ham extension Livewire pages: "Sold this month," "Top earners last 90 days," "Listed > 180 days, no sale" — each one a thin view over the general queries. First slice landed: `Extensions\Ham\AutoParts\Livewire\Insights\SoldThisMonth` resolves the operator's company + default currency and renders the current calendar month's `SalesInsightsService::soldInPeriod` summary as a six-tile card (sales, units, revenue, cost, fees, gross profit). The route is gated by `commerce.marketplace.list` as a temporary fit until a `commerce.sales.view` capability lands. The extension's `ServiceProvider` now registers a `ham-auto-parts` view namespace via `loadViewsFrom`, so templates ship under `extensions/ham/auto-parts/Views/` regardless of `VITE_THEME_DIR`. "Top earners last 90 days" and "Listed > 180 days, no sale" remain.
