...
- [ ] Surface “missing pack for company country” as Payroll readiness data; block final approval/close/export. *(Partial: the calculator already records `no_country_pack` in the run's country-pack audit payload. The remaining work — a hard gate on run close/export through the lock path (`ClosedPayrollRunException`, `PayrollLockAuditReportBuilder`) plus an employee/run readiness signal — is a focused follow-up, deliberately not rushed into this session alongside financial finalization.)*
