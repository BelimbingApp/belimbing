...
- **Never load a whole growing table into PHP** (logs, history, runs, audit): page, chunk, cursor, or ask the database for the aggregate (for example latest-per-key). Mark such models with `GrowingTable`; the hydration guard and the PHPStan rule enforce it — `docs/architecture/query-bounds.md`.
