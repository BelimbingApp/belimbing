...
<thead class="bg-surface-subtle/80">
    <tr>
        ...
        @foreach ($days as $day)
            ...
            @endphp
            <th scope="col" class="{{ $headerSurface }} {{ $colMinWidth }} {{ $colPadding }} py-table-header-y text-center text-[11px] font-semibold uppercase tracking-wider {{ $headerInk }} @if($isToday) underline decoration-accent decoration-2 underline-offset-4 @endif" wire:key="day-strip-{{ $day['date'] }}" @if($isHoliday) title="{{ __('Holiday') }}" @endif>
                <div>{{ $dayLabel }}</div>
                @if (! $compact)
