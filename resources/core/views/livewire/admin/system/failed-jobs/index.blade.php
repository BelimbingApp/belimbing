<div>
    ...
    <div class="space-y-section-gap">
        ...
        <x-ui.card>
            ...
            <x-ui.table container="flush" :caption="__('Failed jobs')">
                <x-slot name="head">
                        <tr>
                            <x-ui.sortable-th
                                column="id"
                            ...
                            <x-ui.sortable-th
                                ...
                                :label="__('Queue')"
                            />
                            <x-ui.th>{{ __('Job Name') }}</x-ui.th>
                            <x-ui.sortable-th
                                column="exception"
                                :sort-by="$sortBy"
                                :sort-dir="$sortDir"
                                action="sort('exception')"
                                :label="__('Exception')"
                            />
                            <x-ui.sortable-th
                                column="failed_at"
                        ...
                        @forelse($failedJobs as $job)
                            <tr wire:key="failed-job-{{ $job->id }}">
                                ...
                                <td class="px-table-cell-x py-table-cell-y whitespace-nowrap text-right">
                                    ...
                                    >
                                        <x-icon name="heroicon-o-arrow-path" class="w-4 h-4" />
                                        <span class="sr-only">{{ __('Retry') }}</span>
                                    </button>
                                    <x-ui.button
