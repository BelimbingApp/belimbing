...
<div>
    ...
    <div class="space-y-section-gap">
        ...
        <x-ui.card>
            ...
            @if ($tab === 'categories')
                @include('livewire.commerce.catalog.partials.categories-workspace')
            @else
                <div class="overflow-x-auto -mx-card-inner px-card-inner">
                    <table class="min-w-full divide-y divide-border-default text-sm">
                    @if ($tab === 'templates')
                        @include('livewire.commerce.catalog.partials.templates-table')
                    @else
                        @include('livewire.commerce.catalog.partials.attributes-table')
                    @endif
                    </table>
                </div>
    ...
    <x-ui.modal wire:model="showCreateModal" class="max-w-2xl">
        @if (($createKind ?: $tab) === 'categories')
            @include('livewire.commerce.catalog.partials.category-form')
        @elseif (($createKind ?: $tab) === 'templates')
            @include('livewire.commerce.catalog.partials.template-form')
        @else
            @include('livewire.commerce.catalog.partials.attribute-form')
        @endif
    </x-ui.modal>
</div>
