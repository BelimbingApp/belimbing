...
>
    ...
    <x-ui.tabs
        tabs-id="foundation-domains-tabs"
        :tabs="[
    ...
    >
        ...
        @if (count($driftedAddInSources) > 0)
            <section id="add-in-source-drift" class="scroll-mt-6 rounded-2xl border border-status-warning-border bg-status-warning-subtle px-4 py-3 text-sm text-status-warning">
                <div class="font-medium">{{ trans_choice('{1} Add-in source has local checkout drift|[2,*] Add-in sources have local checkout drift', count($driftedAddInSources)) }}</div>
                ...
                <div class="mt-3 grid gap-3 md:grid-cols-2">
                    @foreach ($driftedAddInSources as $source)
                        <div class="rounded-lg border border-status-warning-border bg-surface-card px-3 py-2 text-xs text-muted">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                @if ($source->isDirty())
                                    <x-ui.badge variant="warning">{{ trans_choice('{1} :count uncommitted change|[2,*] :count uncommitted changes', (int) $source->workingTree['dirty'], ['count' => $source->workingTree['dirty']]) }}</x-ui.badge>
                                @endif
                                @if ($source->unpushed() > 0)
                                    <x-ui.badge variant="warning">{{ trans_choice('{1} :count unpushed commit|[2,*] :count unpushed commits', $source->unpushed(), ['count' => $source->unpushed()]) }}</x-ui.badge>
                                @endif
                            </div>
                            <div class="mt-1 font-mono">{{ $source->path }}</div>
                            <div class="mt-2 space-y-1 font-mono text-[11px] text-ink">
                                <div>git -C "{{ $source->path }}" status --short</div>
                                @if ($source->unpushed() > 0)
                                    <div>git -C "{{ $source->path }}" log --oneline @@{u}..HEAD</div>
                                @endif
                            </div>
                        </div>
                    @endforeach
        ...
        <x-ui.card x-data="{ open: false }">
            ...
            <p class="mt-1 text-sm text-muted">{{ __('The Platform Baseline: Base infrastructure plus the required Core Domain. Always installed; cannot be disabled or removed.') }}</p>
            @if ($platformSource)
                @if ($platformSource->repo)
                    <p class="mt-2 font-mono text-xs text-muted">{{ $platformSource->repo }} · {{ $platformSource->branch }}@if (! empty($platformSource->commit['short'])) · {{ $platformSource->commit['short'] }}@endif</p>
                @endif
                @if ($platformSource->moduleCount() > 0)
                    <button type="button" x-on:click="open = ! open" class="mt-2 flex items-center gap-1 text-sm text-muted hover:text-ink">
                        ...
                        {{ trans_choice('{1} :count module|[2,*] :count modules', $platformSource->moduleCount(), ['count' => $platformSource->moduleCount()]) }}
                    </button>
                    <div x-show="open" x-collapse class="mt-2 grid gap-2 md:grid-cols-2">
                        @foreach ($platformSource->modules as $m)
                ...
                @endif
                @include('livewire.base.foundation.partials.source-contributions', ['contributions' => $platformSource->contributions])
            @endif
        </x-ui.card>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                @forelse ($installed as $domain)
                    ...
                    <x-ui.card wire:key="domain-{{ $domainName }}" x-data="{ open: false }">
                        ...
                        @php $source = $sourcesByLifecycle[$domainName] ?? null; @endphp
                        @if ($source && $source->repo)
                            <p class="mt-2 font-mono text-xs text-muted">{{ $source->repo }} · {{ $source->branch }}@if (! empty($source->commit['short'])) · {{ $source->commit['short'] }}@endif</p>
                        @endif
                        @if ($source)
                            @include('livewire.base.foundation.partials.source-contributions', ['contributions' => $source->contributions])
                        @endif
                ...
                @empty
                    <x-ui.card>
                        <div class="text-sm text-muted">{{ __('No optional Domains are installed. Install one from the Available tab.') }}</div>
                    </x-ui.card>
                @endforelse
            </div>
        </section>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    @foreach ($extensions as $extension)
                        ...
                        <x-ui.card wire:key="extension-{{ $extName }}" x-data="{ open: false }">
                            ...
                            @php $source = $sourcesByLifecycle[$extName] ?? null; @endphp
                            @if ($source && $source->repo)
                                <p class="mt-2 font-mono text-xs text-muted">{{ $source->repo }} · {{ $source->branch }}@if (! empty($source->commit['short'])) · {{ $source->commit['short'] }}@endif</p>
                            @endif
                            @if ($source)
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    @foreach ($slotSources as $slot)
                        <x-ui.card wire:key="slot-{{ $slot->key }}" x-data="{ open: false }">
                            ...
                            @include('livewire.base.foundation.partials.source-contributions', ['contributions' => $slot->contributions])
                        </x-ui.card>
                    @endforeach
                </div>
            </section>
        ...
        <section class="space-y-2">
            ...
            @if (count($catalogEntries) === 0)
                <x-ui.card>
                    <p class="text-sm text-muted">{{ __('No catalog entries cached. Refresh from GitHub to discover BelimbingApp software sources.') }}</p>
                </x-ui.card>
            @else
                <div class="grid gap-4 md:grid-cols-2">
