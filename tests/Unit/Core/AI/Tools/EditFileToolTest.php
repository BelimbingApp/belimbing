...
use App\Core\AI\Tools\EditFileTool;
...
const EDIT_FILE_TOOL_EXTENSION_DIRECTORY = 'app/Extensions/EditFileTest';
...
it('creates files under a new parent directory', function () {
    ...
    expect($payload['summary'])->toContain('Created '.EDIT_FILE_TOOL_TEST_FILE)
        ->and($payload['target_surface'])->toBe('platform')
...
it('blocks extension paths when target surface is platform', function () {
    $result = executeEditFileTool($this->tool, [
...
it('writes inside selected extension surface', function () {
    ...
    expect($payload['target_surface'])->toBe('extension:edit-file-test')
        ->and($payload['file_path'])->toBe('app/Extensions/EditFileTest/sample.txt')
        ->and($payload['summary'])->toContain('app/Extensions/EditFileTest/sample.txt')
