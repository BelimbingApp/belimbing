...
const RESIDUE_TEST_MIGRATION = '2099_01_01_000000_create_zz_orphan_residue_table';
const RESIDUE_TEST_CONFIRMATION = 'THIS CANNOT BE UNDONE';
const RESIDUE_TEST_DROP_LABEL = 'Permanently drop';
...
function createDatabaseResidueFixture(): void
{
    ...
    DB::table('migrations')->insert([
        'migration' => RESIDUE_TEST_MIGRATION,
        'batch' => 999,
    ]);
...
it('renders the residue page with orphaned tables, ledger rows, and settings', function (): void {
    ...
    $this->get(route('admin.system.database-residue.index'))
        ...
        ->assertSee(RESIDUE_TEST_MIGRATION)
...
it('drops selected orphan tables and prunes their ledger rows after typed confirmation', function (): void {
    ...
    Livewire::test(ResidueIndex::class)
        ...
        ->set('confirmText', RESIDUE_TEST_CONFIRMATION)
    ...
    Livewire::test(ResidueIndex::class)
        ->set('selectedLedger', [RESIDUE_TEST_MIGRATION])
        ->set('confirmText', RESIDUE_TEST_CONFIRMATION)
        ->call('pruneSelectedLedger')
        ->assertHasNoErrors();

    expect(
        DB::table('migrations')->where('migration', RESIDUE_TEST_MIGRATION)->exists()
    )->toBeFalse();
});
...
it('reveals action buttons only when items are selected and the acknowledgment is typed', function (): void {
    ...
    $component = Livewire::test(ResidueIndex::class)
        ->assertDontSee(RESIDUE_TEST_DROP_LABEL);
    ...
    $component->set('confirmText', RESIDUE_TEST_CONFIRMATION)
        ->assertSee(RESIDUE_TEST_DROP_LABEL)
