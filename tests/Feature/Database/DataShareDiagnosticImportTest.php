...
function createDiagnosticImportTables(string $suffix): array
{
    ...
    TableRegistry::register($parent, 'DataShareTest', 'tests/Feature/Database');
    TableRegistry::register($child, 'DataShareTest', 'tests/Feature/Database');
...
it('receives, inspects, and transactionally imports a diagnostic package parents first', function (): void {
    ...
    expect(Artisan::call('blb:db:share:import-diagnostic', ['path' => $capture['path']]))->toBe(0)
    ...
    $receiptPath = 'data-share/incoming/diagnostic/'.$capture['package_id'].'/receipt.json';
    ...
    expect(Artisan::call('blb:db:share:import-diagnostic', [
        'path' => $capture['path'],
        '--commit' => true,
    ]))->toBe(0);
...
it('rejects Incoming bytes that no longer match the destination receipt', function (): void {
    ...
    expect(fn () => app(DiagnosticPackageImporter::class)->inspect($capture['package_id']))
        ->toThrow(DataShareImportException::class, 'no longer matches its receipt hash');
});
...
it('categorically refuses diagnostic receipt and import outside development', function (): void {
    ...
    expect(fn () => app(DiagnosticPackageInbox::class)->receiveLocal($capture['path']))
        ...
        ->and(Storage::disk('local')->files('data-share/incoming'))->toBeEmpty();
