...
use App\Base\Database\Services\Bridge\ColumnRedactor;
use App\Base\Database\Services\Bridge\DependencyClosureResolver;
...
it('forbids capture without the bridge capture capability', function (): void {
    $admin = createAdminUser();
...
it('forbids package inventory without the bridge view capability', function (): void {
    $admin = createAdminUser();
    $plainUser = User::factory()->create(['company_id' => $admin->company_id]);
    $this->actingAs($plainUser);

    $this->get(route('admin.system.database-bridge.index'))->assertForbidden();
});

it('lists and deletes packages on the data bridge page', function (): void {
    $user = createCaptureTargetUser();

    $result = createReviewedDiagnosticCapture('users', [(string) $user->id]);

    $this->get(route('admin.system.database-bridge.index'))
        ->assertOk()
        ->assertSee('Data Bridge')
...
it('refuses to write diagnostic packages to a public disk', function (): void {
    ...
    ))->toThrow(BridgeCaptureException::class, 'disk public is public');
});
