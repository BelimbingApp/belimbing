...
it('does not clobber a policy a controller already set', function (): void {
    ...
    $seed->headers->set('Content-Security-Policy', "default-src 'none'; sandbox");
...
it('withholds HSTS on plaintext requests and sends it over TLS', function (): void {
    ...
    $plain = runSecurityHeaders(Request::create('http://local.blb.lara/dashboard', 'GET'));
    expect($plain->headers->has('Strict-Transport-Security'))->toBeFalse();

    $secure = runSecurityHeaders(Request::create('https://local.blb.lara/dashboard', 'GET'));
