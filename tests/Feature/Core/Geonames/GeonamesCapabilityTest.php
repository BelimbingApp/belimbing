...
test('authenticated user without geonames capabilities gets 403 on all three pages', function (): void {
...
test('list-only user can load all three pages', function (): void {
    $this->withoutVite();
    $this->actingAs(geonamesUser(['admin.geonames.list']));

    $this->get(route('admin.geonames.countries.index'))->assertOk();
    $this->get(route('admin.geonames.admin1.index'))->assertOk();
    $this->get(route('admin.geonames.postcodes.index'))->assertOk();
});

test('list-only user is denied country writes', function (): void {
    $this->actingAs(geonamesUser(['admin.geonames.list']));
    ...
    Livewire::test(CountriesIndex::class)
        ->call('saveName', $country->id, 'Renamed')
...
test('list-only user is denied admin1 writes', function (): void {
    $this->actingAs(geonamesUser(['admin.geonames.list']));
    ...
    Livewire::test(Admin1Index::class)
        ->call('saveName', $admin1->id, 'Renamed')
        ->assertDispatched('notify', variant: 'error', message: $denied);
    ...
    expect($admin1->refresh()->name)->toBe('Division 0');
});

test('list-only user is denied postcode writes', function (): void {
    $this->actingAs(geonamesUser(['admin.geonames.list']));
    $denied = __('You do not have permission to perform this action.');

    Livewire::test(PostcodesIndex::class)
        ->set('selectedCountries', ['SG'])
        ->call('import')
        ->assertDispatched('notify', variant: 'error', message: $denied);
...
test('core admin can rename a country', function (): void {
