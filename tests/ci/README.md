...
The audit checks PR-triggered producers, job display names (falling back to job
IDs), and the required GitHub App identity. SonarCloud Code Analysis is produced
by the Sonar App's scan action, not by a job with the same label. Its actual scan
step must remain in a PR workflow and target SonarCloud.
