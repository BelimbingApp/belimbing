...
- Self-skipping is not the only entry criterion for that lane. A file that runs on both drivers but **proves database semantics** — foreign-key resolution, trigger behaviour, constraint handling, anything a fix was made for — must be registered there too, or the fix is only ever proven against the driver it was not about (belimbing#533: the DataShare package test that proves #528 ran on PostgreSQL only when reviewers ran it by hand). Registration is by file name, and the failure that matters is **omission**, not misspelling: a path that does not exist fails loudly (pest exits 2 with "Test file ... not found" and the step goes red), while a file that was never listed runs on SQLite only, passes there, and nothing reports that it skipped PostgreSQL. The evidence a file runs is the step's test count changing, not a green tick.
- Before proposing any CI/driver change: list the existing workflows and driver branches, and cite what production runs.
- PostgreSQL CI gives each RefreshDatabase-driven Pest run its own unmigrated database, so `migrate:fresh`'s blocked `db:wipe` degrades to a real `migrate` rather than reusing an earlier CI migration. A persistent (non-`:memory:`) database still cannot heal from a test that drops or alters its schema during that run; use an explicit throwaway connection for such a test, and drop and recreate a locally poisoned database.
